...
mpq::~mpq() {
	...
	if(_archive_handle)	SFileCloseArchive(_archive_handle);
}
...
void mpq::extract_to_disk(std::string filename, std::string disk_file) {
  ...
  if(!SFileExtractFile(_archive_handle, filename.c_str(), disk_file.c_str(), SFILE_OPEN_FROM_MPQ)) {

       std::cerr << filename << " could not be extracted to "<< disk_file << std::endl;
